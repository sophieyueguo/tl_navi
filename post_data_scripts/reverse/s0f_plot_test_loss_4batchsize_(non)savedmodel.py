...
NUM_EPISODES = 49
MAX_NUM_SPARKY_INPUT = 6
s0_NAME = 'NoneStartModel Trained with data from Sparky Map 0'

# avg_arima = np.zeros(MAX_NUM_SPARKY_INPUT)
...
#     for i in range(1, MAX_NUM_SPARKY_INPUT+1):
...
for MAP_HARDNESS in HARDNESS_LIST:
    ...
    s0_PATH = 'data/results/reverse/s0f' + MAP_ABBR + '/batchsize4/trainval-f' + MAP_ABBR + '_test-s02trials/test_loss_all.npy'
    ...
    for DOES_TRAIN_FROM_SAVED_MODEL in [False, True]:
        ...
        f_PATH_1 = 'data/results/reverse/s0f' + MAP_ABBR + '/batchsize4/'
        # f_PATH_1 = 'data/results/grid_level/s0f' + MAP_ABBR + '/batchsize4/'
        if not DOES_TRAIN_FROM_SAVED_MODEL:
            f_PATH_1 += 'non'
        f_PATH_1 += 'savedmodel-f' + MAP_ABBR + '_trainval-s0'
        f_PATH_2 = 'trials_test-s02trials/test_loss_all.npy'
    ...
    # # fig = plt.figure(figsize=(7, 4))
    # ind = np.arange(MAX_NUM_SPARKY_INPUT)
    # # width = 0.35
...
# print ('avgs0', avg_s0)
for DOES_TRAIN_FROM_SAVED_MODEL in [False, True]:
    avg_ys[DOES_TRAIN_FROM_SAVED_MODEL] = np.zeros(MAX_NUM_SPARKY_INPUT)
    avg_es[DOES_TRAIN_FROM_SAVED_MODEL] = np.zeros(MAX_NUM_SPARKY_INPUT)
...
df = pd.DataFrame({'Number of Finetuning Trajectories': [0] + list(ind+1) + list(ind+1), \
                    'Type': ['With finetuning (use pre-trained model with 3 Falcon Map Data)']* (1 +len(ind)) + ['Trained from Scratch']*len(ind),\
                    y_name: [avg_s0] + list(avg_ys[True]) + list(avg_ys[False]),\
                    'lable y': [avg_s0] + list(avg_ys[True]+0.01) + list(avg_ys[False]-0.03)})
palette = ['b', 'r']
...
ind_1 = np.arange(NUM_EPISODES)
fig, ax = plt.subplots(figsize=(7, 4))

# print (list(avg_ys[True]) + list(avg_ys[False]))
df_1 = pd.DataFrame({'Training Episodes': list(ind_1+1) + list(ind_1+1), \
                    'Type': ['With finetuning (use pre-trained model with 3 Falcon Map Data)']* len(ind_1) + ['Trained from Scratch']*len(ind_1),\
                    'Average Test Accuracy (among 1-10 Finetuning Trajectories)': list(avg_ys_1[True]) + list(avg_ys_1[False]),\
                    'lable y': list(avg_ys_1[True]+0.001) + list(avg_ys_1[False]-0.001)})
palette = ['b', 'r']
