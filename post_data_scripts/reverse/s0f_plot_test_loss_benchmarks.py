...
df = pd.DataFrame({'Number of Finetuning Trajectories': [0] + list(ind+1) + list(ind+1) + list(ind+1), \
                    'Type': ['With finetuning (use pre-trained model with 3 Falcon Map Data)']* (1 +len(ind)) + ['Trained from Scratch']*len(ind) + ['ARIMA']*len(ind),\
                    y_name: [avg_s0] + list(avg_ys[True]) + list(avg_ys[False]) + list(avg_arima),\
                    'lable y': [avg_s0] + list(avg_ys[True]+0.01) + list(avg_ys[False]-0.03) + list(avg_arima-0.015)})
palette = ['b', 'r', 'g']
...
ind_1 = np.arange(PLOT_NUM_EPI) # want fewer episodes
fig, ax = plt.subplots(figsize=(7, 4))
