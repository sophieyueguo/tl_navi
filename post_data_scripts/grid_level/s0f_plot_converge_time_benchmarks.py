...
# plt.show()
room_level = {}
room_level[True] = np.load('post_data_scripts/grid_level/room_level_converge_time_savedmodel.npy')
room_level[False] = np.load('post_data_scripts/grid_level/room_level_converge_time_nonsavedmodel.npy')

ind = np.arange(MAX_NUM_FALCON_INPUT)
fig, ax = plt.subplots(figsize=(7, 4))
ax.set_ylim([0, 5500])
# print (list(avg_ys[True]) + list(avg_ys[False]))
df_1 = pd.DataFrame({'Number of Finetuning Trajectories': list(ind+1) + list(ind+1) + list(ind+1) + list(ind+1), \
                    'Type': ['Graph Level With finetuning (use pre-trained model with Sparky Data)']* len(ind) + ['Graph Level Trained from Scratch']*len(ind) + ['Grid Level With finetuning (use pre-trained model with Sparky Data)']* len(ind) + ['Grid Level Trained from Scratch']*len(ind),\
                    'Average Runtime Taken for Convergence (seconds)': list(room_level[True][0:MAX_NUM_FALCON_INPUT]) + list(room_level[False][0:MAX_NUM_FALCON_INPUT]) + list(avg_ys_1[True]) + list(avg_ys_1[False]),\
                    'lable y': list(room_level[True][0:MAX_NUM_FALCON_INPUT]-200) + list(room_level[False][0:MAX_NUM_FALCON_INPUT]-200) + list(avg_ys_1[True]-200) + list(avg_ys_1[False]+200)})
# palette = ['b', 'r']
