...
for MAP_HARDNESS in HARDNESS_LIST:
    ...
    s0_PATH = 'data/results/grid_level/s0f' + MAP_ABBR + '/batchsize4/trainval-s0_test-f' + MAP_ABBR + NUM_TEST + 'trials/test_loss_all.npy'
    # s0_PATH = 'data/results/grid_level/s0f' + MAP_ABBR + '/batchsize4/trainval-s0_test-f' + MAP_ABBR + NUM_TEST + 'trials/test_loss_all.npy'
    ...
    for DOES_TRAIN_FROM_SAVED_MODEL in [False, True]:
        ...
        f_PATH_2 = 'trials_test-f' + MAP_ABBR + NUM_TEST + 'trials/test_loss_all.npy'
...
# df = pd.DataFrame({'Number of Finetuning Trajectories': [0] + list(ind+1) + list(ind+1), \
#                     'Type': ['With finetuning (use pre-trained model with Sparky Data)']* (1 +len(ind)) + ['Trained from Scratch']*len(ind),\
#                     y_name: [avg_s0] + list(avg_ys[True]) + list(avg_ys[False]),\
#                     'lable y': [avg_s0] + list(avg_ys[True]+0.01) + list(avg_ys[False]-0.01)})
# palette = ['b', 'r']
...
room_level = {}
room_level[True] = np.load('post_data_scripts/grid_level/room_level_accuracy_savedmodel.npy')
room_level[False] = np.load('post_data_scripts/grid_level/room_level_accuracy_nonsavedmodel.npy')

# print (list(avg_ys[True]) + list(avg_ys[False]))
df_1 = pd.DataFrame({'Training Episodes': list(ind_1+1) + list(ind_1+1) + list(ind_1+1) + list(ind_1+1), \
                    'Type':  ['Graph Level With finetuning (use pre-trained model with Sparky Data)']* len(ind_1) + ['Graph Level Trained from Scratch']*len(ind_1) + ['Grid Level With finetuning (use pre-trained model with Sparky Data)']* len(ind_1) + ['Grid Level Trained from Scratch']*len(ind_1),\
                    'Average Test Accuracy (among 1-10 Finetuning Trajectories)': list(room_level[True][0:PLOT_NUM_EPI]) + list(room_level[False][0:PLOT_NUM_EPI]) + list(avg_ys_1[True][0:PLOT_NUM_EPI]) + list(avg_ys_1[False][0:PLOT_NUM_EPI]),\
                    'lable y':list(room_level[True][0:PLOT_NUM_EPI]) + list(room_level[False][0:PLOT_NUM_EPI]) +  list(avg_ys_1[True][0:PLOT_NUM_EPI]+0.001) + list(avg_ys_1[False][0:PLOT_NUM_EPI]-0.005)})
