...
for MAP_HARDNESS in HARDNESS_LIST:
    ...
    s0_PATH = 'data/results/room_level/s0f' + MAP_ABBR + '/batchsize4/trainval-s0_test-f' + MAP_ABBR + NUM_TEST + 'trials/test_loss_all.npy'
    ...
    for DOES_TRAIN_FROM_SAVED_MODEL in [False, True]:
        ...
        f_PATH_1 = 'data/results/room_level/s0f' + MAP_ABBR + '/batchsize4/'
...
df = pd.DataFrame({'Number of Finetuning Trajectories': [0] + list(ind+1) + list(ind+1) + list(ind+1) + list(ind+1),\
                    'Type': ['TL-DCRNN with finetuning (use pre-trained model with Sparky Data)']* (1 +len(ind)) + ['TL-DCRNN Trained from Scratch']*len(ind) + ['ARIMA']*len(ind) + ['Rule-based']*len(ind),\
                    y_name: [avg_s0] + list(avg_ys[True]) + list(avg_ys[False]) + list(avg_arima) + list(avg_rule_base),\
                    'lable y': [avg_s0] + list(avg_ys[True]+0.01) + list(avg_ys[False]-0.015) + list(avg_arima-0.015) + list(avg_rule_base-0.02)})
palette = ['b', 'r', 'g', 'brown']
...
# np.save('post_data_scripts/grid_level/room_level_accuracy_savedmodel', avg_ys_1[True])
# np.save('post_data_scripts/grid_level/room_level_accuracy_nonsavedmodel', avg_ys_1[False])
# print (list(avg_ys[True]) + list(avg_ys[False]))
df_1 = pd.DataFrame({'Training Episodes': list(ind_1+1) + list(ind_1+1), \
                    'Type': ['With finetuning (use pre-trained model with Sparky Data)']* len(ind_1) + ['Trained from Scratch']*len(ind_1),\
                    ...
                    'lable y': list(avg_ys_1[True][0:PLOT_NUM_EPI]+0.01) + list(avg_ys_1[False][0:PLOT_NUM_EPI]-0.01)})
palette = ['b', 'r']
