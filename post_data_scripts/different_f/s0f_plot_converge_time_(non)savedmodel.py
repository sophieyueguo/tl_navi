...
# MAP_ABBR = 'e'
    ...
    # DOES_TRAIN_FROM_SAVED_MODEL = True
        ...
        f_PATH_2 = 'trials_test-f' + MAP_ABBR + MAP_NUM_TEST_TRIALS[MAP_ABBR] + 'trials/'
...
df = pd.DataFrame({'Number of Finetuning Trajectories': list(ind+1) + list(ind+1), \
                    'Type': ['With finetuning (use pre-trained model with Falcon Med Map Data)']* len(ind) + ['Trained from Scratch']*len(ind),\
...
plt.grid(True)
plt.title('Runtime Taken for Convergence Varying Number of Trajectory Inputs \n(Graph Level, Test on Falcon Easy and Hard Maps)')
plt.show()
