...
max_trail_count = 5
h5_name = 'testbed_fe_train_' + str(max_trail_count) + 'trials'
# h5_name = 'testbed_fe_test_' + str(max_trail_count) + 'trials'
...
def simulate_single_traj(level, data_path, knowledge_condition, vis_window=False):
    ...
    for t in range(len(positions)): # read the message one by one, paused time ignored
        ...
        if F_XMIN <= x <= F_XMAX and F_ZMIN <= z <= F_ZMAX:
            x, z = int(x), int(z)
            # print ('x', x, 'z', z)
            x = x - x % SCALE - F_XMIN
            z = z - z % SCALE - F_ZMIN
            ...
            if x < 0:
                x += SCALE
            feature[ids.index('fx' + str(x) + 'z' + str(z))] = 1
            # print ('fx' + str(x) + 'z' + str(z))
