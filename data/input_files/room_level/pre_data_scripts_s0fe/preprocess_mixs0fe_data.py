...
h5_path = 'transfer_learning/TL-DCRNN-master/data/input_files/sparky-0_falcon-e/h5/'

fe_traj_max_count = 5
h5_name = 'triagesignal_fe_count' + str(fe_traj_max_count)
...
filter_freq = 20 # take 10% of the sparky data, change the density, just as for the testbed



def simulate_falcon_single_traj(level, data_path, knowledge_condition, vis_window=False):
...
def simulate_sparky_single_traj(data_path, trial):
    f = open(data_path, "r")
...
if __name__ == '__main__':
    ...
    for level in ['Easy']:
        for jsonfile in jsonfiles:
            if level in jsonfile and '-TriageSignal' in jsonfile:
                ...
                single_trial_graph_feature, ts_list = simulate_falcon_single_traj(abbr[level], data_path, knowledge_condition, vis_window=False)
    ...
    for txtfile in txtfiles:
        if '_0' in txtfile:
            ...
            single_trial_graph_feature, ts_list = simulate_sparky_single_traj(data_path, trial)
            # print ('len(single_trial_graph_feature)', len(single_trial_graph_feature))
            # print ('len(ts_list)', len(ts_list))
            # print ()
            multi_trial_graph_features += single_trial_graph_feature
